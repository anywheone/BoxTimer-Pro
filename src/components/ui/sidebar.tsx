      ...
      <SidebarContext.Provider value={contextValue}>
        <TooltipProvider delayDuration={0}>
          <div
            ...
            className={cn(
              "group/sidebar-wrapper flex-1 min-h-svh has-[[data-variant=inset]]:bg-sidebar",
              className
            )}
            ref={ref}
            {...props}
  ...
  (
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            "fixed top-24 bottom-0 z-40 hidden h-[calc(100vh-6rem)] w-[--sidebar-width] transition-[left,right,width] duration-200 ease-linear md:flex",
            side === "left"
...
>(({ className, ...props }, ref) => {
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    setIsResizing(true)
    setStartX(e.clientX)
    setStartWidth(parseFloat(sidebarWidth.replace('rem', '')))
  }
  ...
  React.useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const clampedWidth = Math.max(
        parseFloat(SIDEBAR_MIN_WIDTH.replace('rem', '')),
        Math.min(parseFloat(SIDEBAR_MAX_WIDTH.replace('rem', '')), newWidth)
      )

      setSidebarWidth(`${clampedWidth}rem`)
    }
  ...
  return (
    <div
      ...
      className={cn(
        "absolute top-0 right-0 w-1 h-full cursor-col-resize z-30 bg-transparent hover:bg-sidebar-border/50 transition-colors",
